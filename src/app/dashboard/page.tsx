...
export default async function DashboardPage({
  ...
  const months = Math.min(24, Math.max(1, Number(monthsParam ?? "12") || 12));
  ...
  const stats = await getDashboardStats(session.user.id, months);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Your job search at a glance — last {months} month{months === 1 ? "" : "s"}.
          </p>
        </div>
        <RangeSelect months={months} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-6">
        <StatCard label="Applications" value={stats.totalApplications} icon={Briefcase} />
        <StatCard label="Active" value={stats.activeApplications} icon={ListChecks} />
        <StatCard label="Interviews" value={stats.interviews} icon={Users} />
        <StatCard label="Assessments" value={stats.assessments} icon={CalendarCheck} />
        <StatCard label="Offers" value={stats.offers} icon={Award} />
        <StatCard label="Rejections" value={stats.rejections} icon={XCircle} />
      </div>
